...
export class BladeParser {
    ...
    private readonly extendsRegex = /@extends\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/;
    ...
    async buildCompleteTree(selectedFilePath: string): Promise<BladeTreeItem | null> {
        try {
            ...
            const chainNodes: BladeTreeItem[] = [rootItem];
            let current: BladeTemplate | null = selectedTemplate;
            let level = 1;
            while (current && current.extendsPath) {
                const parentTemplate: BladeTemplate | null = await this.parseBladeFile(current.extendsPath);
                if (!parentTemplate) { break; }
                const parentItem: BladeTreeItem = {
 ...
 */
function getConfiguredLaravelRoot(): string | undefined {
    try {
        const cfgAny: any = vscode.workspace.getConfiguration('laravelBladeVisualizer') as any;
        let value: unknown;
        if (cfgAny && typeof cfgAny.get === 'function') {
            value = cfgAny.get('laravelRoot');
        } else if (cfgAny && typeof cfgAny === 'object') {
            value = (cfgAny as any)['laravelRoot'];
        }
 ...
 */
function detectProjectRootFrom(currentFilePath: string): string | undefined {
    ...
    for (const folder of workspaceFolders) {
        ...
        if (fs.existsSync(viewsPath)) { return candidate; }
    }
    return undefined;
}
 ...
 */
function getViewsRootFor(currentFilePath: string): string {
 ...
 */
function computeDisplayName(filePath: string): string {
